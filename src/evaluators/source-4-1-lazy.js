...
function evaluate(stmt, env) {
   return is_self_evaluating(stmt)
        ...
        : is_head_statement(stmt)
          ? eval_head_statement(stmt, env)
        : is_tail_statement(stmt)
          ? eval_tail_statement(stmt, env)
        : is_pair_statement(stmt) 
          ? eval_pair_statement(operands(stmt),env)
        : is_application(stmt)
...
parse_and_eval("function factorial(x) {\
                return n === 1\
                ? 1\
                : factorial(n - 1) * n;}");
