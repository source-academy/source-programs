...
function run() {
    while (RUNNING) {
        if (M[P[PC]] === undefined) {
            ...
            _PC = PC;
            M[P[PC]]();
            assert_valid_node(OS, list("assert valid OS after PC: " + stringify(_PC)));
            assert_correct_env(ENV, list("assert valid ENV after PC: " + stringify(_PC)));
        }
    }
    if (STATE === DIV_ERROR) {
        POP_OS();
